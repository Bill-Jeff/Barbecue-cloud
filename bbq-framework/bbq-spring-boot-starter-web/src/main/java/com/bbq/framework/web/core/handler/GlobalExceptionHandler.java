package com.bbq.framework.web.core.handler;
...
import com.bbq.framework.common.exception.ServiceException;
import com.bbq.framework.common.exception.util.ServiceExceptionUtil;
import com.bbq.framework.common.pojo.CommonResult;
import com.bbq.framework.common.util.collection.SetUtils;
import com.bbq.framework.common.util.json.JsonUtils;
import com.bbq.framework.common.util.monitor.TracerUtils;
import com.bbq.framework.common.util.servlet.ServletUtils;
import com.bbq.framework.web.core.util.WebFrameworkUtils;
...
import static com.bbq.framework.common.exception.enums.GlobalErrorCodeConstants.*;
...
public class GlobalExceptionHandler {
    ...
    public CommonResult<?> defaultExceptionHandler(HttpServletRequest req, Throwable ex) {
        ...
        // 例如说：https://gitee.com/zhijiantianya/bbq-cloud/issues/ICSSRM、https://gitee.com/zhijiantianya/bbq-cloud/issues/ICT6FM
    ...
    private CommonResult<?> handleTableNotExists(Throwable ex) {
        ...
        if (message.contains("report_")) {
            log.error("[报表模块 bbq-module-report - 表结构未导入][参考 https://cloud.iocoder.cn/report/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[报表模块 bbq-module-report - 表结构未导入][参考 https://cloud.iocoder.cn/report/ 开启]");
        }
        // 2. 工作流
        if (message.contains("bpm_")) {
            log.error("[工作流模块 bbq-module-bpm - 表结构未导入][参考 https://cloud.iocoder.cn/bpm/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[工作流模块 bbq-module-bpm - 表结构未导入][参考 https://cloud.iocoder.cn/bpm/ 开启]");
        }
        // 3. 微信公众号
        if (message.contains("mp_")) {
            log.error("[微信公众号 bbq-module-mp - 表结构未导入][参考 https://cloud.iocoder.cn/mp/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[微信公众号 bbq-module-mp - 表结构未导入][参考 https://cloud.iocoder.cn/mp/build/ 开启]");
        }
        // 4. 商城系统
        if (StrUtil.containsAny(message, "product_", "promotion_", "trade_")) {
            log.error("[商城系统 bbq-module-mall - 已禁用][参考 https://cloud.iocoder.cn/mall/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[商城系统 bbq-module-mall - 已禁用][参考 https://cloud.iocoder.cn/mall/build/ 开启]");
        }
        // 5. ERP 系统
        if (message.contains("erp_")) {
            log.error("[ERP 系统 bbq-module-erp - 表结构未导入][参考 https://cloud.iocoder.cn/erp/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[ERP 系统 bbq-module-erp - 表结构未导入][参考 https://cloud.iocoder.cn/erp/build/ 开启]");
        }
        // 6. WMS 仓库管理系统
        if (message.contains("wms_")) {
            log.error("[WMS 仓库管理系统 bbq-module-wms - 表结构未导入][参考 https://cloud.iocoder.cn/wms/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[WMS 仓库管理系统 bbq-module-wms - 表结构未导入][参考 https://cloud.iocoder.cn/wms/build/ 开启]");
        }
        // 7. CRM 系统
        if (message.contains("crm_")) {
            log.error("[CRM 系统 bbq-module-crm - 表结构未导入][参考 https://cloud.iocoder.cn/crm/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[CRM 系统 bbq-module-crm - 表结构未导入][参考 https://cloud.iocoder.cn/crm/build/ 开启]");
        }
        // 8. MES 系统
        if (message.contains("mes_")) {
            log.error("[MES 系统 bbq-module-mes - 表结构未导入][参考 https://cloud.iocoder.cn/mes/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[MES 系统 bbq-module-mes - 表结构未导入][参考 https://cloud.iocoder.cn/mes/build/ 开启]");
        }
        // 9. IM 即时通讯
        if (message.contains("im_")) {
            log.error("[IM 即时通讯 bbq-module-im - 表结构未导入][参考 https://cloud.iocoder.cn/im/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[IM 即时通讯 bbq-module-im - 表结构未导入][参考 https://cloud.iocoder.cn/im/build/ 开启]");
        }
        // 10. 支付平台
        if (message.contains("pay_")) {
            log.error("[支付模块 bbq-module-pay - 表结构未导入][参考 https://cloud.iocoder.cn/pay/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[支付模块 bbq-module-pay - 表结构未导入][参考 https://cloud.iocoder.cn/pay/build/ 开启]");
        }
        // 11. AI 大模型
        if (message.contains("ai_")) {
            log.error("[AI 大模型 bbq-module-ai - 表结构未导入][参考 https://cloud.iocoder.cn/ai/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[AI 大模型 bbq-module-ai - 表结构未导入][参考 https://cloud.iocoder.cn/ai/build/ 开启]");
        }
        // 12. IoT 物联网
        if (message.contains("iot_")) {
            log.error("[IoT 物联网 bbq-module-iot - 表结构未导入][参考 https://doc.iocoder.cn/iot/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[IoT 物联网 bbq-module-iot - 表结构未导入][参考 https://doc.iocoder.cn/iot/build/ 开启]");
        }
        return null;
    }
