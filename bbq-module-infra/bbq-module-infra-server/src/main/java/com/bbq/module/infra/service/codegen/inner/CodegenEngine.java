package com.bbq.module.infra.service.codegen.inner;
...
import com.bbq.framework.apilog.core.annotation.ApiAccessLog;
import com.bbq.framework.apilog.core.enums.OperateTypeEnum;
import com.bbq.framework.common.exception.util.ServiceExceptionUtil;
import com.bbq.framework.common.pojo.CommonResult;
import com.bbq.framework.common.pojo.PageParam;
import com.bbq.framework.common.pojo.PageResult;
import com.bbq.framework.common.util.collection.CollectionUtils;
import com.bbq.framework.common.util.date.DateUtils;
import com.bbq.framework.common.util.date.LocalDateTimeUtils;
import com.bbq.framework.common.util.object.BeanUtils;
import com.bbq.framework.common.util.object.ObjectUtils;
import com.bbq.framework.common.util.string.StrUtils;
import com.bbq.framework.excel.core.annotations.DictFormat;
import com.bbq.framework.excel.core.convert.DictConvert;
import com.bbq.framework.excel.core.util.ExcelUtils;
import com.bbq.framework.mybatis.core.dataobject.BaseDO;
import com.bbq.framework.mybatis.core.mapper.BaseMapperX;
import com.bbq.framework.mybatis.core.query.LambdaQueryWrapperX;
import com.bbq.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import com.bbq.module.infra.dal.dataobject.codegen.CodegenTableDO;
import com.bbq.module.infra.enums.codegen.CodegenFrontTypeEnum;
import com.bbq.module.infra.enums.codegen.CodegenSceneEnum;
import com.bbq.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import com.bbq.module.infra.enums.codegen.CodegenVOTypeEnum;
import com.bbq.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 bbq-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 bbq-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.bbq.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/bbq-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/bbq-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "bbq-module-${table.moduleName}/" + // 顶级模块
                "bbq-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "bbq-module-${table.moduleName}/" + // 顶级模块
                "bbq-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "bbq-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "bbq-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "bbq-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "bbq-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
